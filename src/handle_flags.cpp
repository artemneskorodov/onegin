...
static const program_mode_t DEFAULT_MODE ={
    .input_file_name = "Onegin.txt"              ,
    .output_file_name = "output.txt"             ,
    .compare_function = string_compare_alphabetic};
...
exit_code_t try_read_file(parsed_text_t *text, program_mode_t *mode) {
    ...
    switch(read_file(mode->input_file_name, text)) {
        case UNKNOWN_READING_ERROR: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Unknown reading error (or assert).\n");
        ...
        case READING_ALLOCATION_ERROR: {
            free_text(text);
            color_printf(YELLOW_TEXT, true, DEFAULT_BACKGROUND,
                         "Error while allocating memmory.\n");
            return EXIT_CODE_FAILURE;
        }
        default: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Unexpected read_file(...) result.\n");
            return EXIT_CODE_FAILURE;
        }
    }
}
...
exit_code_t try_parse_text(parsed_text_t *text, program_mode_t *) {
    ...
    switch(parse_strings(text)) {
        case UNKNOWN_PARSING_ERROR: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Unexpected error while parsing text into strings.\n");
            return EXIT_CODE_FAILURE;
        }
        case PARSING_SUCCESS: {
            color_printf(YELLOW_TEXT, true, DEFAULT_BACKGROUND,
                         "Successfully parsed text into strings.\n");
            return EXIT_CODE_SUCCESS;
        }
        ...
        case PARSING_BEFORE_READING_INPUT: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Parsing text into lines before reading lines.\n");
            return EXIT_CODE_FAILURE;
        }
        default: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Unexpected return value of parse_strings(...).\n");
            return EXIT_CODE_FAILURE;
        }
    }
}
...
exit_code_t try_sort_text(parsed_text_t *text, program_mode_t *mode) {
    ...
    if(sort_array(text->lines, sizeof(char *), text->strings_number,
                  mode->compare_function) != SORTING_SUCCESS) {

        color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                     "Error while sorting lines.\n");
        return EXIT_CODE_FAILURE;
    }
...
exit_code_t try_write_output(parsed_text_t *text, program_mode_t *mode) {
    ...
    switch(write_file(mode->output_file_name, text)) {
        case UNKNOWN_WRITING_ERROR: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Caught unsupported error, while running write_file(...).\n");
            return EXIT_CODE_FAILURE;
        }
        case WRITING_SUCCESS: {
            free_text(text);
            color_printf(GREEN_TEXT, true, DEFAULT_BACKGROUND,
                         "Successfully wrote sorted text to file '%s'.\n", mode->output_file_name);
            return EXIT_CODE_SUCCESS;
        }
        ...
        case WRITING_APPENDING_ERROR:{
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Error while writing text to file.\n");
            return EXIT_CODE_FAILURE;
        }
        default: {
            free_text(text);
            color_printf(RED_TEXT, true, DEFAULT_BACKGROUND,
                         "Unexpected return value for write_file(...).\n");
            return EXIT_CODE_SUCCESS;
        }
    }
}
